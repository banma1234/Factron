...
const init = () => {
    ...
    document.querySelectorAll('.filter-tab').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            this.classList.add('active'); // 현재 버튼 active

            applyFilter(this.dataset.status); // 선택한 탭 상태로 필터링
        });
    });
    ...
    approvalGrid.on('dblclick', (e) => {
        ...
        if (!rowData || !rowData.approvalId) return; // 데이터 없으면 리턴
 ...
 * @param selectedStatus : 'ALL' | 'APPROVED' | 'APV001' 등
 */
function applyFilter(selectedStatus) {
    ...
    approvalGrid.resetData(filtered); // 그리드 갱신
}

/**
 * 단일 항목 승인/반려 후 새로고침 시 사용
 * approvalId 로 서버에서 다시 조회 후 rawApprovalData 갱신
 */
window.refreshSingleApproval = async (approvalId) => {
    try {
        ...
        // 발령(APR003)이면 발령자 이름/사번을, 아니면 요청자 이름/사번을 표시
...
window.getData = async () => {
    ...
    if ((startDate && !endDate) || (!startDate && endDate)) {
        alert("시작 및 종료 날짜를 모두 입력해주세요."); return;
    }
    ...
    try {
        ...
        // 발령(APR003)이면 발령자, 아니면 요청자 정보로 표시 이름/사번 결정
        ...
        approvalGrid.resetData(modified); // 그리드에 반영
    } catch (e) {
